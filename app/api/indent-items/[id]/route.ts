import { NextRequest, NextResponse } from 'next/server'
import { getServerSession } from 'next-auth'
import { authOptions } from '@/lib/auth'
import { query, formatDatetime } from '@/lib/db'

export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  const session = await getServerSession(authOptions)
  if (!session) return NextResponse.json({ success: false }, { status: 401 })
  const userId = (session.user as { id: string }).id
  const { category_id, product_id, order_stock } = await req.json()
  await query('UPDATE indent_items SET category_id=?,product_id=?,order_stock=?,updated_by=?,updated_on=? WHERE indent_item_id=?',
    [category_id, product_id, order_stock, userId, formatDatetime(), params.id])
  return NextResponse.json({ success: true })
}

export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  try {
    const session = await getServerSession(authOptions)
    if (!session) return NextResponse.json({ success: false }, { status: 401 })
    const userId = (session.user as { id: string }).id
    const items = await query<{ product_id: number; order_stock: number }[]>('SELECT product_id, order_stock FROM indent_items WHERE indent_item_id=?', [params.id])
    if (!items.length) return NextResponse.json({ success: true })
    const { product_id, order_stock } = items[0]
    await query('UPDATE inventory_items SET available_stock=available_stock+?,updated_by=?,updated_on=? WHERE product_id=?',
      [order_stock, userId, formatDatetime(), product_id])
    await query('DELETE FROM indent_items WHERE indent_item_id=?', [params.id])
    return NextResponse.json({ success: true })
  } catch (err: any) {
    return NextResponse.json({ success: false, message: err.message || 'Failed to delete' }, { status: 500 })
  }
}
